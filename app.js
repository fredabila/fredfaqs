...
const firebaseConfig = {
  ...
  function loadUserQuestions() {
    ...
    userQuestions.forEach((q) => {
      ...
      div.innerHTML = `
        ...
        <p class="mt-2 answer" data-id="${q.id}">${q.answered ? `Answer: ${q.answer}` : "Waiting for answer..."}</p>
      `;
  ...
  function getTimerText(question) {
    ...
    return remaining > 0 ? `Time remaining: ${remaining} seconds` : "Time's up! Please wait for an admin to respond.";
  }
  
  function startTimer(questionId, initialTimeLeft, timerStarted) {
    const timerElement = document.querySelector(`.timer[data-id="${questionId}"]`);
    const answerElement = document.querySelector(`.answer[data-id="${questionId}"]`);
    ...
    const updateTimer = () => {
      ...
      if (remaining > 0) {
        ...
        timerElement.textContent = "Time's up! Please wait for an admin to respond.";
        answerElement.textContent = "Waiting for an admin to respond. Thank you for your patience.";
        clearInterval(timerId);
      }
    };
      ...
      .onSnapshot((snapshot) => {
        ...
        snapshot.forEach((doc) => {
          ...
          li.innerHTML = `
            ...
            <button class="bg-green-500 text-white px-4 py-2 rounded-md hover:bg-green-600 transition duration-300 ease-in-out" onclick="submitAnswer('${doc.id}', this)">Submit Answer</button>
          `;
